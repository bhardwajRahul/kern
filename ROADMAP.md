...
| **How far down the privilege ladder a containerised kern still runs** | It runs inside a container today, and CI holds it there on `main`, on the listed branches and on every pull request: `kern box --memory 64m` inside a privileged `docker run`, red if the cap does not bite AND red if, where the controller cannot be delegated, `doctor` and the box disagree about that. What is NOT measured is how far down from `--privileged` it goes: then the default seccomp, then the default AppArmor, then a read-only cgroup, then without `SYS_ADMIN`. The lowest rung that still runs is the pod spec a security team would be asked to sign |
